function [theta_t, pos_t, sum_int] = n_particles_to_use_reflec (n, r_spon, r_align, sigma_theta, dt, n_iter, ...
...
for t = 2:n_iter
    ...
    for i = 1:n
        ...
        if (pos(i,1) < 0)
                pos(i,1) = -pos(i,1);
                theta(i) = atan2(sin(theta(i)), -cos(theta(i))); % Introducing reflective boundary
        elseif (pos(i,1) > box_length)
                pos(i,1) = (2*box_length) - pos(i,1);
        ...
        if (pos(i,2) < 0)
                pos(i,2) = -pos(i,2);
                theta(i) = atan2(-sin(theta(i)), cos(theta(i)));
        elseif (pos(i,2) > box_length)
                pos(i,2) = (2*box_length) - pos(i,2);
                theta(i) = atan2(-sin(theta(i)), cos(theta(i)));
        end
        if theta(i) < 0
