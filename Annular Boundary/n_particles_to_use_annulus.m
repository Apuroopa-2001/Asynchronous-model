function [theta_t, pos_t, sum_int] = n_particles_to_use_annulus (n, r_spon, r_align, sigma_theta, dt, n_iter, ...
    k_alg, S0, inner_R, outer_R, int_rad)

% INITIAL CONDITIONS - Putting agents in a lattice

radial_pos(:) = rand(1, n) * (outer_R - inner_R) + inner_R;
...
for t = 2:n_iter
    ...
    for i = 1:n
        ...
        if (r_x_1_y_1 == inner_R || r_x_1_y_1 == outer_R) % Introducing reflective boundary
            x_0 = init_pos(1);
            y_0 = init_pos(2);
            ...
            y_2 = pos(i,2);
            circle_R = inner_R;
            if (r_x_1_y_1 == outer_R)
                circle_R = outer_R;
            end
            theta(i) = reflectPointOnC(x_0, y_0, x_1, y_1, x_2, y_2, circle_R);
            
        elseif (r_x_1_y_1 < inner_R || r_x_1_y_1 > outer_R) % Introducing reflective boundary
            x_0 = init_pos(1);
            y_0 = init_pos(2);
            x_1 = pos(i,1);
            y_1 = pos(i,2);
            ...
            index = 0;
            if (r_x_1_y_1 < inner_R)
                while (dist_reflec_point <= inner_R && index == 0)
                    if (dist_reflec_point < inner_R)
                        [x_0, y_0, x_1, y_1, theta(i)] = reflectPointOutC(x_0, y_0, x_1, y_1, inner_R);
                        dist_reflec_point = sqrt(x_1^2 + y_1^2);
                    elseif (dist_reflec_point == inner_R)
                        x_2 = x_1;
                        y_2 = y_1;
                        x_1 = x_1 + cos(theta(i));
                        y_1 = y_1 + sin(theta(i));
                        theta(i) = reflectPointOnC(x_0, y_0, x_1, y_1, x_2, y_2, inner_R);
                        index = 1;
                    end
                end
            else
                while (dist_reflec_point >= outer_R && index == 0)
                    if (dist_reflec_point > outer_R)
                        [x_0, y_0, x_1, y_1, theta(i)] = reflectPointOutC(x_0, y_0, x_1, y_1, outer_R);
                        dist_reflec_point = sqrt(x_1^2 + y_1^2);
                    elseif (dist_reflec_point == outer_R)
                        x_2 = x_1;
                        y_2 = y_1;
                        x_1 = x_1 + cos(theta(i));
                        y_1 = y_1 + sin(theta(i));
                        theta(i) = reflectPointOnC(x_0, y_0, x_1, y_1, x_2, y_2, outer_R);
                        index = 1;
                    end
                end
            end
